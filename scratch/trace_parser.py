...
print(f"=== CHAR-BY-CHAR PARSER TRACE (L3870 to L3895) ===")
...
while char_idx < len(script_code):
    ...
    log_char = (3870 <= current_line <= 3895)
    if log_enabled := log_char:
    ...
    if was_in_string and char_idx < len(script_code):
        # If we were in a string, we skip template literal checks for this character
        # (unless it was the quote closing the string, in which case we still skip it because it's part of the string)
        char_idx += 1
        current_col += 1
        continue
    ...
    if char == '`':
        if stack and stack[-1] == 'backtick':
            stack.pop()
            if log_enabled:
                print(f"  >>> POP BACKTICK. Stack: {stack}")
        else:
            stack.append('backtick')
            if log_enabled:
                print(f"  >>> PUSH BACKTICK. Stack: {stack}")
            
    elif char == '$' and peek(2) == '${':
        if stack and stack[-1] == 'backtick':
            stack.append('placeholder')
            if log_enabled:
                print(f"  >>> PUSH PLACEHOLDER. Stack: {stack}")
            char_idx += 1
            current_col += 1
        ...
        if stack and stack[-1] == 'placeholder':
            stack.pop()
            if log_enabled:
                print(f"  >>> POP PLACEHOLDER. Stack: {stack}")
